...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, size, activation, momentum, classes_num = 5):
...
class EncoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, downsample, activation, momentum, classes_num = 5):
...
class DecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride, activation, momentum, classes_num = 5):
...
# class ZeroShotASP():
    ...
    def __init__(self, config, channels=5):
        super().__init__()
    ...
    def forward(self, input, condition):
        ...
        (x_pool, x4) = self.encoder_block4(x_pool, condition)    # x4_pool: (bs, 256, T / 16, F / 16)
        # (x_pool, x5) = self.encoder_block5(x_pool, condition)    # x5_pool: (bs, 512, T / 32, F / 32)
        # (x_pool, x6) = self.encoder_block6(x_pool, condition)    # x6_pool: (bs, 1024, T / 64, F / 64)
        x_center = self.conv_block7(x_pool, condition)    # (bs, 2048, T / 64, F / 64)
        # x = self.decoder_block1(x_center, x6, condition)  # (bs, 1024, T / 32, F / 32)
        # x = self.decoder_block2(x_center, x5, condition)    # (bs, 512, T / 16, F / 16)
        x = self.decoder_block3(x_center, x4, condition)    # (bs, 256, T / 8, F / 8)
        x = self.decoder_block4(x, x3, condition)   # (bs, 128, T / 4, F / 4)
        x = self.decoder_block5(x, x2, condition)  # (bs, 64, T / 2, F / 2)
        x = self.decoder_block6(x, x1, condition)  # (bs, 32, T, F)
        x = self.after_conv_block1(x, condition)     # (bs, 32, T, F)
...
class LAAS(pl.LightningModule):
    ...
    def forward(self, input):
        ...
        x, mixed_audio = self.ASP(input=input["audio"], condition=input["conditions"])
        temp_wav_out = x['wav']
        ...
        out = {"separated_audio_features": audio_features, "mixed_audio": mixed_audio}
        # do other things
        return out
