...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses,
                 num_points, oversample_ratio, importance_sample_ratio, batch_size):
    ...
    def loss_masks(self, outputs, indices, num_masks):
        """Compute the losses related to the masks: clip_loss, scoremap_loss
        """
        assert "pred_masks" in outputs

        # src_idx = self._get_src_permutation_idx(indices)
        # tgt_idx = self._get_tgt_permutation_idx(indices)
        src_mask_embedding = outputs["mask_embedding"] # batch_size * num_queries * hidden_dim
        src_mask_features = outputs["mask_features"] # batch_size * hidden_dim * hidden_dim * hidden_dim
        src_masks = outputs["pred_masks"] # batch_size * num_queries * hidden_dim * hidden_dim
        ...
        losses = {
            "scoremap_loss": Scoremap_loss(src_masks),
            "clip_loss": CLIP_loss(src_mask_embedding, src_mask_features, self.batch_size),
            # "loss_mask": sigmoid_ce_loss_jit(point_logits, point_labels, num_masks),
            # "loss_dice": dice_loss_jit(point_logits, point_labels, num_masks),
        }

        del src_masks, src_mask_embedding, src_mask_features
        # del target_masks
        return losses
    ...
    def forward(self, outputs, targets):
        ...
        for loss in self.losses:
            losses.update(self.get_loss(loss, outputs, indices, num_masks))
